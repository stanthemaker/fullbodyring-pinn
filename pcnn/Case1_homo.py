...
class PhysicsInformedNN:
    ...
    def predict_eval(self, epoch, figname):
        ...
        # data = np.concatenate(U_pinn_pred, axis=0)
        # np.savez_compressed("/home/stan/data/pinn/data/pred.npz", data=data)
...
if __name__ == "__main__":
    ...
    data_size = wave_data[indices[0]].shape[0]

    u_ini1 = interpolate.griddata(
        xz_0, wave_data[indices[0]].reshape(-1), xz_ini, fill_value=0.0
    )
    u_ini2 = interpolate.griddata(
        xz_0, wave_data[indices[1]].reshape(-1), xz_ini, fill_value=0.0
    )
    # u_ini1 = zoom(
    #     wave_data[slice1_idx], (n_ini / data_size, n_ini / data_size), order=1
    # )
    # u_ini2 = zoom(
    #     wave_data[slice2_idx], (n_ini / data_size, n_ini / data_size), order=1
    # )
    ...
    u_ini2_s2 = interpolate.griddata(
    ...
    u_eval1 = interpolate.griddata(
        xz_0, wave_data[indices[0]].reshape(-1), xz_eval, fill_value=0.0
    )
    u_eval2 = interpolate.griddata(
        xz_0, wave_data[indices[1]].reshape(-1), xz_eval, fill_value=0.0
    )
    u_eval3 = interpolate.griddata(
        xz_0, wave_data[indices[2]].reshape(-1), xz_eval, fill_value=0.0
    )
    u_eval4 = interpolate.griddata(
        xz_0, wave_data[indices[3]].reshape(-1), xz_eval, fill_value=0.0
    )
    ...
    for i in range(1):
        ...
        checkpoints_path = (
            f"{save_checkpoints_dir}/loop_{i}_ini_LBFGS_checkpoints_8000.dump"
        )
        # checkpoints_path = (
        #     "/home/stan/data/pinn/pcnn/5hz_conti/ckpt/loop_0_adam_checkpoints_30000.dump"
        # )
        model = PhysicsInformedNN(model_Dir=checkpoints_path, log_file=log_file)
        model.train_adam(
            n_iters=100001, calc_NTK=True, update_lambda=True, IfIni=False, loop_iter=i
        )
