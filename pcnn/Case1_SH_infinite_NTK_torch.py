...
class PhysicsInformedNN:
    def __init__(self, ExistModel=0, model_Dir="", log_file=""):
        ...
        for epoch in range(n_iters):
            ...
            if self.adam_iter % 500 == 0:
                ...
                with open(self.log_file, "a") as f:
                    ...
                    f.write(
                        f"loop_iter {self.loop_iter}, Adam Iter {self.adam_iter}, Loss: {loss.item():.4f}, loss_ini: {loss_ini.item():.4f}, loss_pde: {loss_pde.item():.4f}\n"
                    )
                    f.write(
    ...
    def closure(self):
        ...
        if self.LBFGS_iter % 200 == 0:
            with open(self.log_file, "a") as f:
                f.write(
                    f"LBFGS: Iter {self.LBFGS_iter}, Loss: {loss_LBFGS.item():4f}, loss_ini: {loss_ini_LBFGS.item():.4f}, loss_pde: {loss_pde_LBFGS.item():.4f}\n"
                )
...
def get_args():
    ...
    parser.add_argument(
        "--cuda", "-c", type=str, default="cuda:0", help="choose a cuda"
    )
...
if __name__ == "__main__":
    ...
    save_checkpoints_dir = f"/home/stan/data/pinn/pcnn/{args.name}/ckpt"
    ...
    u_ini1 = np.load("/home/stan/data/pinn/data/wavefields/u_ini1.npz")["arr_0"]
    u_ini2 = np.load("/home/stan/data/pinn/data/wavefields/u_ini2.npz")["arr_0"]
    ...
    # # uploading the wavefields from specfem
    # u_ini1_s2 = interpolate.griddata(
    #     xz_spec, U0[0], xz_ini_s2, fill_value=0.0
    # )  # [1600, 2]
    ...
    u_ini1_s2 = np.load("/home/stan/data/pinn/data/wavefields/u_ini1_s2.npz")["arr_0"]
    u_ini2_s2 = np.load("/home/stan/data/pinn/data/wavefields/u_ini2_s2.npz")["arr_0"]
    # wavefields for eval
    n_eval = 100
    ...
    # u_eval1_0 = interpolate.griddata(
    #     xz_spec, U0[0], xz_eval, fill_value=0.0
    # )  # [1600, 2]
    ...
    u_eval1 = np.load("/home/stan/data/pinn/data/wavefields/u_eval1.npz")["arr_0"]
    u_eval2 = np.load("/home/stan/data/pinn/data/wavefields/u_eval2.npz")["arr_0"]
    u_eval3 = np.load("/home/stan/data/pinn/data/wavefields/u_eval3.npz")["arr_0"]
    u_eval4 = np.load("/home/stan/data/pinn/data/wavefields/u_eval4.npz")["arr_0"]
    ...
    for i in range(1):
        checkpoints_path = (
            f"{save_checkpoints_dir}/loop_{i}_ini_LBFGS_checkpoints_4000.dump"
        )
        model = PhysicsInformedNN(log_file=log_file)
        ...
        model = PhysicsInformedNN(
            ExistModel=1, model_Dir=checkpoints_path, log_file=log_file
        )
        model.train_adam(
            n_iters=10001, calc_NTK=True, update_lambda=True, IfIni=False, loop_iter=i
        )
