...
class PhysicsInformedNN:
    def __init__(self, model_Dir="", log_file="", map_file="", lbfgs_iter=8000):
        ...
        self.sos_map = np.load(map_file)["data"] / xz_scl
    ...
    def train_adam(
        ...
        for epoch in range(n_iters):
            ...
            if calc_NTK & (epoch % 200 == 0):
                ...
                X_ini2_NTK[:, 2] = X_ini2_NTK[:, 2] * 0 + (time_pts[1] - time_pts[0])
    ...
    def predict_eval(self, epoch, figname):
        ...
        for i in range(len(X_evals)):
            ...
            p_eval = self.predict(X_evals[i])
            P_PINN = p_eval.detach().cpu().numpy()

            # Compute the difference
            P_diff = p_evals[i] - P_PINN
...
if __name__ == "__main__":
    ...
    for i in range(len(indices)):
        time_pts.append((indices[0] / n_slices) * time_span)
    ...
    for i in range(1):
        ...
        # )
        model = PhysicsInformedNN(
            model_Dir=checkpoints_path, log_file=log_file, map_file=map_file
        )
        model.train_adam(
